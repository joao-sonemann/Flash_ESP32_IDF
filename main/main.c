...
typedef struct{
    char palavra2[SSID_MAX_LEN];
} label2_t;

typedef struct{
    char palavra3[SSID_MAX_LEN];
} label3_t;
...
void map_partition() {
    ...
    while (iterator != NULL) {
            ...
            char* nome_label;
            nome_label = (char*) malloc(sizeof(char) * (strlen("label1") + 1));
            strcpy(nome_label, "label1");

            ESP_LOGI(TAG, "nome_label: %s", nome_label);

            if(strcmp(nome_label, part->label) == 0){
